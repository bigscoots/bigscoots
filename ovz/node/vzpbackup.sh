...
if grep bksvr "$BSPATH"/nodebackupinfo >/dev/null 2>&1 ; then
  BKSVR=$(grep bksvr "$BSPATH"/nodebackupinfo | sed 's/bksvr=//g')
else
    echo "" | mail -s "$HOSTNAME has no backup server defined. Mark for Justin" monitor@bigscoots.com
    exit
fi

if grep bkuser "$BSPATH"/nodebackupinfo >/dev/null 2>&1 ; then
  BKUSER=$(grep bkuser "$BSPATH"/nodebackupinfo | sed 's/bkuser=//g')
else
    BKUSER=bigscoots
fi
...
if grep -w "$CTID" <<< `$VZLIST_CMD -a -Hoctid` &> /dev/null; then
        ...
        if [ "$COMPRESS" == "tgz" ]; then
            ...
            # Fastest compression
            tar -cvf -  --exclude='./root.hdd/root.hdd.*' --exclude='./root.hdd/root.hds.*' . | ssh "${BKUSER}"@"${BKSVR}" "gzip -1 > ~/vps/$(hostname | cut -d"." -f1)/vzpbackup_${CTID}_${HNAME}_${TIMESTAMP}.tar.gz"
            # No compression
