...
if grep -q tim@localhost.localdomain /vz/root/"$1"/root/.ssh/authorized_keys; then

	sed -i '/tim@localhost.localdomain/d ; /Office 4 - BigScoots.com/d' /vz/root/"$1"/root/.ssh/authorized_keys
fi

if grep -q bassu@Thunderstorm /vz/root/"$1"/root/.ssh/authorized_keys; then

	sed -i '/bassu@Thunderstorm/d ; /Office 5 - BigScoots.com/d' /vz/root/"$1"/root/.ssh/authorized_keys
fi
...
if ! grep -q "${office04}" /vz/root/"$1"/root/.ssh/authorized_keys; then
 ...
 if ! grep -q "${office05}" /vz/root/"$1"/root/.ssh/authorized_keys; then
  ...
  if ! grep -q "${office06}" /vz/root/"$1"/root/.ssh/authorized_keys; then
	echo "# Office 6 - BigScoots.com" >> /vz/root/"$1"/root/.ssh/authorized_keys
	echo "${office06}" >> /vz/root/"$1"/root/.ssh/authorized_keys
 fi
