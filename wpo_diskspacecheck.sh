...
if [[ $1 = ALL ]] ; then
    ...
    while read size unit ; do tdusize=$size ; tduunit=$unit ; done <<< $(du -sc --exclude 'ib_logfile*' --exclude 'ibdata*' /var/lib/mysql/ /home/nginx/domains/ | tail -1 | awk '{print $1}'| awk '{ split( "K M G" , v ); s=1; while( $1>1024 ){ $1/=1024; s++ } print int($1) v[s] }' | grep -Eo '[[:alpha:]]+|[0-9.?]+')
    ...
    while read size unit ; do tdusize=$size ; tduunit=$unit ; done <<< $(du -sc /var/lib/mysql/"$(grep DB_NAME /home/nginx/domains/"$1"/public/wp-config.php | grep -v WP_CACHE_KEY_SALT | grep -v '^//' | cut -d \' -f 4)" /home/nginx/domains/"$1" | tail -1 | awk '{print $1}'| awk '{ split( "K M G" , v ); s=1; while( $1>1024 ){ $1/=1024; s++ } print int($1) v[s] }' | grep -Eo '[[:alpha:]]+|[0-9.?]+')
