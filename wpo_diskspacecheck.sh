...
if [[ $1 = ALL ]] ; then
    ...
    while read size unit ; do tdusize=$size ; tduunit=$unit ; done <<< $(du -hc --exclude 'ib_logfile*' --exclude 'ibdata*' /var/lib/mysql/ /home/nginx/domains/ | tail -1 | awk '{print $1}' | sed -r 's/([0-9])([a-zA-Z])/\1 \2/g')
    ...
    while read size unit ; do tdusize=$size ; tduunit=$unit ; done <<< $(du -hc /var/lib/mysql/"$(grep DB_NAME /home/nginx/domains/"$1"/public/wp-config.php | grep -v WP_CACHE_KEY_SALT | grep -v '^//' | cut -d \' -f 4)" /home/nginx/domains/"$1" | tail -1 | awk '{print $1}'| sed -r 's/([0-9])([a-zA-Z])/\1 \2/g')
