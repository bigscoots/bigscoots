...
liveserver=192.168.1.196
 ...
 ssh -p 2222 "$liveserver" "whmapi1 listaccts" |grep user: |awk '{print $2}' >> /tmp/remoteusers.txt 
...
xferid=$(whmapi1 create_remote_root_transfer_session remote_server_type=cpanel host="$liveserver" port=2222 user=root sshkey_name=id_rsa transfer_threads=10 restore_threads=10 unrestricted_restore=1 copy_reseller_privs=0 compressed=0 unencrypted=0 low_priority=0 |grep transfer_session_id: | awk '{print $2}')
...
for cpuser in $(ssh -p 2222 "$liveserver" "whmapi1 listaccts" |grep user: |awk '{print $2}') ; do 
